...
**爬虫日志中的"筛选出 N 条"不可用于向用户报告数据量。必须用数据库实际入库数量。**

| 日志用语 | 含义 | 能否用于报告 |
|----------|------|:--:|
| "筛选出 N 条职教政策" | 标题关键词匹配的链接数 | ❌ 不可 |
| "新增: X, 跳过(重复): Y" | 本次运行实际入库/跳过 | ⚠️ 增量 |
| `SELECT COUNT(*) FROM Policy` | 数据库真实总数 | ✅ 唯一定量 |

**原因**: 从"筛选出"到入库，中间经过正文验证、日期过滤、外链去重等多层过滤，过滤率可达 80-95%。以"筛选出"数量报告给用户是误导。

**操作**: 每次爬虫运行后，必须执行 `check-counts.ts` 核实各省份实际入库数量，以此为准向用户汇报。

### 政策分类原则

**类型无法明确判定时，标记为 `M-其他`，不得强行归入 A-治理体系。**
当前 A-治理体系占比 65.5% 的主要原因是 classifyPolicy() 将其作为 default 兜底类型。
